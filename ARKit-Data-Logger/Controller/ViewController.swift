...
class ViewController: UIViewController, ARSCNViewDelegate, ARSessionDelegate {
    ...
    private func startVideoRecording() {
        ...
        // let videoOutputURL = createVideoOutputURL()
        
        // // Set video recording settings (e.g., resolution, frame rate, etc.)
        // let videoSettings = VideoSettings()
        // videoSettings.size = CGSize(width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height)
        
        // videoRecorder.startRecording(fileName: videoOutputURL.path, settings: videoSettings)

        videoRecorder.record()
    }
    
    private func stopVideoRecording() {
        videoRecorder?.stopAndExport { [weak self] (session) in
            guard let self = self else { return }
            
            if let videoURL = session?.exportURL {
    ...
    private func createVideoOutputURL() -> URL {
        ...
        let videoOutputFilename = "recordedVideo.mp4"
        let videoOutputURL = documentsDirectory.appendingPathComponent(videoOutputFilename)
        
        if fileManager.fileExists(atPath: videoOutputURL.path) {
            do {
                try fileManager.removeItem(at: videoOutputURL)
            } catch {
                print("Failed to remove previous video file: \(error)")
            }
        }
    ...
    func session(_ session: ARSession, didUpdate frame: ARFrame) {
        ...
        self.customQueue.async {
            if ((self.fileHandlers.count == self.numTextFiles) && self.isRecording) {
                ...
                self.videoRecorder?.update(frame)
